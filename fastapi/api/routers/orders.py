...
@router.get('')
async def read_orders(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
...
@router.get('/{user_id}')
async def read_orders_by_user_id(user_id: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.post('')
async def create_order(order: PedidoSchema, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def delete_order_by_id(order_id: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
